...
$numrows=$DB->query("SELECT * from pay_order WHERE pid={$pid}{$sql}")->rowCount();
$pagesize=30;
...
$list=$DB->query("SELECT * FROM pay_order WHERE pid={$pid}{$sql} order by addtime desc limit $offset,$pagesize")->fetchAll();
	...
	<div class="panel panel-default">
	  ...
	  <div class="row wrapper">
        ...
        <table class="table table-striped">
          <thead><tr><th>交易号/商户订单号</th><th>商品名称</th><th>商品金额</th><th>支付方式</th><th>创建时间/完成时间</th><th>状态</th></tr></thead>
          <tbody>
<?php
foreach($list as $res){
	echo '<tr><td>'.$res['trade_no'].'<br/>'.$res['out_trade_no'].'</td><td>'.$res['name'].'</td><td>￥ <b>'.$res['money'].'</b></td><td> <b>'.$res['type'].'</b></td><td>'.$res['addtime'].'<br/>'.$res['endtime'].'</td><td>'.($res['status']==1?'<font color=green>已完成</font>':'<font color=red>未完成</font>').'</td></tr>';
}
?>
		  </tbody>
        </table>
